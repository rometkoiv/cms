...
/**
 * Home object for domain model class ProjectType.
 * @see .ProjectType
 * @author Hibernate Tools
 */
public class ProjectTypeHome {

	private static final Log log = LogFactory.getLog(ProjectTypeHome.class);
	...
	public void persist(ProjectType transientInstance) {
		log.debug("persisting ProjectType instance");
		try {
	...
	public ProjectType merge(ProjectType detachedInstance) {
		...
		try {
			ProjectType result = (ProjectType) sessionFactory
	...
	public ProjectType findById(java.math.BigDecimal id) {
		log.debug("getting ProjectType instance with id: " + id);
		try {
			ProjectType instance = (ProjectType) sessionFactory
					.getCurrentSession().get("ProjectType", id);
			if (instance == null) {
